...
C3rdPoundingState::C3rdPoundingState()
	: m_QTEUI(nullptr)
	, m_iQTECount (30)
	, m_iPrevQTECount (30)
{
...
void C3rdPoundingState::finaltick()
{
	if (m_QTEUI == nullptr)
	{
	...
	if (m_iQTECount != m_iPrevQTECount)
	{
		...
		if (m_iQTECount == 20)
		{
			pSelf->GetScript<CBossScript>()->SetBossState(BOSS_STATE::PhaseFinal_3rdPoundingReady_Two);
			pMainCamera->GetScript<CCameraScript>()->ZoomInWithTime(0.05f, 0.1f);
		}
		if (m_iQTECount == 10)
		{
			pSelf->GetScript<CBossScript>()->SetBossState(BOSS_STATE::PhaseFinal_3rdPoundingReady_Three);
			pMainCamera->GetScript<CCameraScript>()->ZoomInWithTime(0.05f, 0.1f);
		}
		if (m_iQTECount <= 0)
		{
			ChangeState(L"BeforeExctionState");
			pMainCamera->GetScript<CCameraScript>()->ZoomInWithTime(0.05f, 0.1f);
		}
	}
...
void C3rdPoundingState::Enter()
{
}

void C3rdPoundingState::Exit()
{
	CGameObject* pSelf = GetFSM()->GetStateMachine()->GetOwner();
	pSelf->GetScript<CBossScript>()->SetBossState(BOSS_STATE::PhaseFinal_3rdPounding);

	GamePlayStatic::Play2DSound(L"sound\\Boss\\Sfx_SNB_ExecuteHit3.wav", 1, 0.1f);
	...
	pObj->Transform()->SetRelativePos(Vec3(pSelf->Transform()->GetRelativePos().x - 20.f, 150.f, pSelf->Transform()->GetRelativePos().z));
	...
	CGameObject* pMainCamera = CLevelMgr::GetInst()->GetCurrentLevel()->FindObjectByName(L"MainCamera");
	pMainCamera->GetScript<CCameraScript>()->ZoomOutWithTime(0.15f, 0.1f);
