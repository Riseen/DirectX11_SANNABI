	...
	, m_iParticleTexNum(0)
{
	...
	m_Module.vSpawnColor = Vec4(0.01f, 0.9f, 0.9f, 1.f);
	...
	m_ParticleTex[0] = CAssetMgr::GetInst()->Load<CTexture>(L"texture\\particle\\Flare.png"
														, L"texture\\particle\\Flare.png");
...
CParticleSystem::CParticleSystem(const CParticleSystem& _OriginParticle)
	...
	, m_ParticleTex{ _OriginParticle.m_ParticleTex[0] }
	, m_Time(0.f)
	, m_iParticleTexNum(_OriginParticle.m_iParticleTexNum)
{
...
void CParticleSystem::render()
{
	...
	GetMaterial()->SetTexParam(TEX_PARAM::TEX_0, m_ParticleTex[m_iParticleTexNum]);
...
void CParticleSystem::SetParticleTexture(const wstring& _strKey, const wstring& _strRelativePath)
{
	m_iParticleTexNum = 1;
}
...
void CParticleSystem::SaveToFile(FILE* _File)
{
	...
	// 파티클 입자 텍스쳐 정보 저장
	SaveAssetRef(m_ParticleTex[0], _File);
}
...
void CParticleSystem::LoadFromFile(FILE* _File)
{
	...
	// 파티클 입자 텍스쳐 정보 로드
	LoadAssetRef(m_ParticleTex[0], _File);
}